...
def cvxsolve(params, v):
    ...
    #s2 = sum((X*np.matmul(gen.todense(),gen.todense()) for X, gen in zip(x, alg_gen_l)))
    ...
    z = block_real_imag_state(v)
    #### Something around here is wrong. Somehow with my new matrices the problem is not well defined, but how should I change that?
    problem = cp.Problem(cp.Minimize(cp.trace(cp.square(z-cp.trace(z@s)*idm))), constraint)
    #### almost sure this is not what I want. I want S(a^2)-S(a)^2 = S((a-S(a))^2) but how do I get there
...
def coordinate_dispersion(matrices,state,sq_mat=0):
    if type(sq_mat)!=int:
        esqs=[np.conj(state)@m@state for m in sq_mat]
    else:
        esqs=[np.conj(state)@m@m@state for m in matrices]

    sqes=[(np.conj(state)@m@state)**2 for m in matrices]

    return abs(np.sum(esqs)-np.sum(sqes))
...
class DistanceParameters:
    ...
    def __init__(self, alg_generators,alg_generators_com_D, D, cvxsolver_args={}, **kwargs):
        ...
        self.alg_gen_l, self.d_alg_gen_l = parse_alg_generators(alg_generators, alg_generators_com_D)
        self.D = D
        self.complex_size = alg_generators[0].shape[0]
        self.x0 = np.ones(self.complex_size)
        self.cvxsolver_args = cvxsolver_args
        self.size = 2*self.complex_size
...
def parse_alg_generators(alg_generators, alg_generators_com_D):
    ...
    alg_gen_asnp = (mat for mat in alg_generators)
